...
namespace EmeraldAI.Utility
{
    ...
    public class EmeraldMovementEditor : Editor
    {
        ...
        //Enums
	    SerializedProperty WanderTypeProp, WaypointTypeProp, AlignAIWithGroundProp, CurrentMovementStateProp, AnimatorTypeProp, AlignmentQualityProp, AlignAIOnStartProp,AWMoveTypeProp,AWPoseTypeProp;
        ...
        //Objects
	    SerializedProperty WaypointObjectProp,WaypointsListProp,WaypointExtendsListProp;
        #endregion
        ...
        void InitializeProperties ()
        {
	        ...
	        AWMoveTypeProp= serializedObject.FindProperty("AWMoveType");
		    AWPoseTypeProp= serializedObject.FindProperty("AWPoseType");
	        ...
	        WaypointsListProp= serializedObject.FindProperty("WaypointsList");
		    WaypointExtendsListProp = serializedObject.FindProperty("WaypointExtendsList");
        }
	    ...
	    void DrawAdvancedWaypointsList()
	    {
		    for (int index = 0; index < WaypointExtendsListProp.arraySize; index++)
		    {
		    	...
		    	SerializedProperty WaypointsListElement = WaypointsListProp.GetArrayElementAtIndex(index);
			    SerializedProperty AdvancedWaypointElement = WaypointExtendsListProp.GetArrayElementAtIndex(index);
			    ...
			    EditorGUILayout.PropertyField(WaypointsListElement, new GUIContent("位置"));
            ...
            if (self.WanderType == EmeraldMovement.WanderTypes.Waypoints)
            {
                ...
                if (WaypointsFoldout.boolValue)
                {
                    ...
                    if (self.WaypointsList != null && Selection.objects.Length == 1)
                    {
                        ...
                        else if (self.WaypointType == (EmeraldMovement.WaypointTypes.Advanced))
                        {
	                        CustomEditorProperties.CustomHelpLabelField("扩展的关键点模式，可以根据扩展的运动和行为信息较为精确地控制AI运动轨迹和行为。", false);
                        }
                        EditorGUILayout.Space();
                        EditorGUILayout.Space();
                        EditorGUILayout.Space();
                        ...
                        if (GUILayout.Button("Import Waypoint Data") && EditorUtility.DisplayDialog("Import Waypoint Data?", "Are you sure you want to clear all of this AI's waypoints and import waypoints from the applied Waypoint Object? This process cannot be undone.", "Yes", "Cancel"))
                        {
	                        ...
	                        self.WaypointExtendsList = new List<WaypointExtend>(self.m_WaypointObject.WaypointExtends);
                        ...
                        if (GUILayout.Button("Export Waypoint Data"))
                        {
                            ...
                            if (SavePath != string.Empty)
                            {
	                            ...
	                            m_WaypointObject.WaypointExtends = new List<WaypointExtend>(self.WaypointExtendsList);
                        ...
                        if (GUILayout.Button("Add Waypoint"))
                        {
	                        ...
	                        self.WaypointExtendsList.Add(new WaypointExtend());
                            EditorUtility.SetDirty(self);
                        }
                        ...
                        if (GUILayout.Button("Clear All Waypoints", style) && EditorUtility.DisplayDialog("Clear Waypoints?", "Are you sure you want to clear all of this AI's waypoints? This process cannot be undone.", "Yes", "Cancel"))
                        {
	                        ...
	                        self.WaypointExtendsList.Clear();
                            EditorUtility.SetDirty(self);
                        }
            ...
            if (self.WanderType == EmeraldMovement.WanderTypes.Waypoints && WaypointsFoldout.boolValue && !HideSettingsFoldout.boolValue)
            {
                if (self.WaypointsList.Count > 0 && self.WaypointsList != null)
                {
                    ...
                    for (int i = 0; i < self.WaypointsList.Count; i++)
                    {
                        ...
                        Handles.color = Color.white;
                        CustomEditorProperties.DrawString("点 " + (i + 1), self.WaypointsList[i] + Vector3.up, Color.white);
                    }
                }
            }
        }
