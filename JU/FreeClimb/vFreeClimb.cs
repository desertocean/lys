...
namespace Invector.vCharacterController.vActions
{
  ...
  public static GameObject CreateTemporarySphere(Vector3 position, float radius, Color color, float duration)
		...
		{   
			var climbUpConditions =  vertical > 0f ;
			//Debug.Log(autoClimbEdge+" "+climbUpConditions);
			//Debug.Log(canMoveClimb +" "+inClimbUp+" "+climbUpConditions +" "+ ignoreInput+" "+(!canMoveClimb && !inClimbUp && (climbUpConditions || ignoreInput)));
		...
		protected virtual void OnAnimatorIK()
		{
			...
			if (ikWeight > 0)
			{
				var lRoot = transform.InverseTransformPoint(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.LeftHand).position);
				var rRoot = transform.InverseTransformPoint(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.RightHand).position);
				RaycastHit hit2;

				if (Physics.Raycast(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.LeftHand).position + transform.forward * -0.5f + transform.up * -0.2f, transform.forward, out hit2, 1f, climbSurfaceLayers))
				{
					targetPositionL = transform.InverseTransformPoint(hit2.point);
					if (debugRays && debugHandIK) Debug.DrawLine(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.LeftHand).position + transform.forward * -0.5f + transform.up * -0.2f, hit2.point, Color.green);
				}
				else
				{
					...
					if (debugRays && debugHandIK) Debug.DrawRay(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.LeftHand).position + transform.forward * -0.5f + transform.up * -0.2f, transform.forward, Color.red);
				}

				if (Physics.Raycast(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.RightHand).position + transform.forward * -0.5f + transform.up * -0.2f, transform.forward, out hit2, 1f, climbSurfaceLayers))
				{
					targetPositionR = transform.InverseTransformPoint(hit2.point);
					if (debugRays && debugHandIK) Debug.DrawLine(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.RightHand).position + transform.forward * -0.5f + transform.up * -0.2f, hit2.point, Color.green);
				}
				else
				{
					...
					if (debugRays && debugHandIK) Debug.DrawRay(TPSCharacter.anim.GetBoneTransform(HumanBodyBones.RightHand).position + transform.forward * -0.5f + transform.up * -0.2f, transform.forward, Color.red);
				}
				...
				TPSCharacter.anim.SetIKPositionWeight(AvatarIKGoal.LeftHand, ikWeight);
				TPSCharacter.anim.SetIKPositionWeight(AvatarIKGoal.RightHand, ikWeight);

				TPSCharacter.anim.SetIKPosition(AvatarIKGoal.LeftHand, leftHandPosition);
				TPSCharacter.anim.SetIKPosition(AvatarIKGoal.RightHand, rightHandPosition);
			}
			else
			{
				TPSCharacter.anim.SetIKPositionWeight(AvatarIKGoal.LeftHand, 0);
				TPSCharacter.anim.SetIKPositionWeight(AvatarIKGoal.RightHand, 0);
			}
		}
