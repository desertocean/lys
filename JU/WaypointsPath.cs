...
{	public enum AWMoveTypes { Walk = 0, Run=1 ,Crawl=2 };
	...
	public class AdvanceWaypoint
	{
		...
		public AdvanceWaypoint(Vector3 Position,AWMoveTypes MoveType = AWMoveTypes.Walk, AWPoseTypes PoseType = AWPoseTypes.Stand, int GrenadeAmount=0,int FireAmount=0)
		{
	...
	public class WaypointPathEditor : Editor
	{
		...
		private void HandleRightClick()
		{
			...
			if (e.type == EventType.MouseDown && e.button == 1)
			{
				...
				menu.AddItem(new GUIContent("Add Point"), false, () => 
				{
					...
					{
						waypointsPath.AdvanceWaypointsList.Add(new AdvanceWaypoint(hit.point));
					}
					else
					{
						...
						{
							waypointsPath.AdvanceWaypointsList.Add(new AdvanceWaypoint(ray.GetPoint(distance)));
						}
						else
						{
							waypointsPath.AdvanceWaypointsList.Add(new AdvanceWaypoint(waypointsPath.transform.position +ray.direction * 5));
						}
					}
		...
		void DrawAdvancedWaypointsList(WaypointsPath self)
		{
			if (GUILayout.Button("Add Waypoint"))
			{
				...
				self.AdvanceWaypointsList.Add(new AdvanceWaypoint(newPoint));
			...
			for (int index = 0; index < AdvanceWaypointsListProp.arraySize; index++)
			{
				...
				if (index < AdvanceWaypointsListProp.arraySize - 1)
				{
					if (GUILayout.Button(new GUIContent("插入", "Inserts a point between this point and the next point."), EditorStyles.miniButton, GUILayout.Height(18)))
					{
						...
						self.AdvanceWaypointsList.Insert(index + 1,new AdvanceWaypoint((self.AdvanceWaypointsList[index].Position + self.AdvanceWaypointsList[index + 1].Position) / 2f));
						EditorUtility.SetDirty(self); 
						HandleUtility.Repaint();
					}
				}
